...
/*CONSTANT*/
#define BUFSIZE_RUN_COMMAND 1024
...
const char * FILE_WIADS_DAEMON_CONFIG = "/etc/config/wiads_daemon";
const char * FILE_HOSTS = "/etc/hosts";
const char * FILE_VERSION = "/etc/version";
const char * FILE_ADDRESS = "/sys/class/net/wlan0/address";
const char * FILE_DHCPLOG = "/tmp/dhcpmasq.log";

const char CMD_REBOOT_MORE1DAY[] = "/root/reboot_more1day.sh";
...
const char CMD_IW_STATION[] = "iw wlan0 station dump | grep 'Station'";
const char CMD_PROC_NF_CONNTRACK[] = "cat /proc/net/nf_conntrack";
...
char * get_api_url(int i) {
	char * option = "option api1";
	if (i == 2) {
		option = "option api2";
	}
	char * content = file_read_line_contain_key(FILE_WIADS_DAEMON_CONFIG, option);
...
//char * get_api_url1() {
//	char * content = file_read_line_contain_key(FILE_WIADS_DAEMON_CONFIG, "option api1");
//	content = str_replace("option api1", " ", content);
//	trim(content);
//	return content;
//}
//
//char * get_api_url2() {
//	char * content = file_read_line_contain_key(FILE_WIADS_DAEMON_CONFIG, "option api2");
//	content = str_replace("option api2", " ", content);
//	trim(content);
//	return content;
//}

char * get_api_status(int i) {
	char * option = "option api1_status";
	if (i == 2) {
		option = "option api2_status";
	}
	char * content = file_read_line_contain_key(FILE_WIADS_DAEMON_CONFIG, option);
	content = str_replace(option, " ", content);
	trim(content);
...
//char * get_api1_status() {
//	char * content = file_read_line_contain_key(FILE_WIADS_DAEMON_CONFIG, "option api1_status");
//	if (content == NULL) {
//		return NULL;
//	}
//	content = str_replace("option api1_status", " ", content);
//	trim(content);
//	return content;
//}
//
//char * get_api2_status() {
//	char * content = file_read_line_contain_key(FILE_WIADS_DAEMON_CONFIG, "option api2_status");
//	content = str_replace("option api2_status", " ", content);
//	trim(content);
//	return content;
//}

char * get_bw_profile() {
	char * content = file_read_line_contain_key(FILE_WIADS_DAEMON_CONFIG, "option bw_profile");
	content = str_replace("option bw_profile", " ", content);
	trim(content);
	return content;
}
...
char * get_conn_station() {
	char * split = ",";
	char * info_output = run_command(CMD_IW_STATION);
	if (info_output == NULL || strlen(info_output) == 0) {
		return NULL;
	}
	struct splits_array * splits = str_splits(info_output, "\n");
	int info_length = strlen(split);
	for (int i = 0; i < splits->arr_len; i++) {
	    char * s = splits->arr[i];
	    struct splits_array * s_plits = str_splits(s, " ");
	    char * mac = s_plits->arr[1];
	    trim(mac);
	    info_length += strlen(mac) + strlen(split);
	}
	info_length += 1;
	char * content = (char *)malloc(info_length);
	strcpy(content, " ");
	for (int i = 0; i < splits->arr_len; i++) {
		char * s = splits->arr[i];
		struct splits_array * s_plits = str_splits(s, " ");
...
void change_default_network_lan_ipaddr() {
    ...
    if (str_equal(network_lan, "option network_lan '192.168.1.1'")
    		|| str_equal(network_lan, "option network_lan 192.168.1.1")) {
        printf("change_default_network_lan_ipaddr \n");
        file_change_key_value_after_segment(FILE_NETWORK_CONFIG,
        		"option ipaddr", option_lan_ipaddr, "config interface 'lan'");
        system("/etc/init.d/network reload");
    }
}

char * get_conn_ccu() {
	int conn = 0, udp = 0, tcp = 0, other = 0;
	char ifname[16];
	char * conn_ccu_str = (char*) malloc(32);
	char line[BUFSIZE_RUN_COMMAND];
	FILE *fp;
	int first_line = 1;
	if ((fp = popen(CMD_PROC_NF_CONNTRACK, "r")) == NULL) {
	      printf("Error opening pipe!\n");
	      return "-1";
	}

	while (fgets(line, BUFSIZE_RUN_COMMAND, fp) != NULL) {
	...
	conn = udp + tcp + other;
	strcpy(conn_ccu_str, itoa(conn, (char*) malloc(8)));
	strcat(conn_ccu_str, ",");
	strcat(conn_ccu_str, itoa(tcp, (char*) malloc(8)));
	strcat(conn_ccu_str, ",");
	strcat(conn_ccu_str, itoa(udp, (char*) malloc(8)));
	return conn_ccu_str;
}
