...
struct http_response* http_req(char *http_headers, struct parsed_url *purl)
{
	...
	while((recived_len = recv(sock, BUF, BUFSIZ-1, 0)) > 0)
	{
        BUF[recived_len] = '\0';
//        if (_DEBUG_) {
//            printf("Before Realloc revived_len %d, response length %d\n", recived_len, strlen(response));
//        }
//		response = (char*)realloc(response, strlen(response) + strlen(BUF) + 1);
//		if (_DEBUG_) {
//		    printf("After Realloc response size %d\n", strlen(response));
//		}
        if (first_buf == 0) {
        	first_buf = 1;
        	strcpy(response, BUF);
...
//	printf("Reallocate response size %d\n", strlen(response));
...
struct http_response* http_get(char *url, char *custom_headers)
{
	...
	/* Declare variable */
	char *http_headers = (char*)malloc(4096);
...
/*
 Makes a HTTP GET request to the given url
 */
char * http_get_302_redirecturl(char *url, char *custom_headers)
{
    /* Parse url */
    struct parsed_url *purl = parse_url(url);
    if(purl == NULL)
    ...
    if(hresp->status_code_int > 300 && hresp->status_code_int < 399)
    {
        ...
        while(token != NULL)
        {
            if(str_contains(token, "Location:"))
            {
                /* Extract url */
                char *location = str_replace("Location: ", "", token);
                int url_size = strlen("http://") + strlen(purl->host) + strlen(location) + 1;
                char * url = (char*) malloc(url_size * sizeof(char));
                printf("http_get_302_redirecturl location %s\n", location);
                if (strx_startwith(location, "/")) {
                    strcpy(url, "http://");
                    strcat(url, purl->host);
                    strcat(url, location);
                } else {
                    url = location;
                }
