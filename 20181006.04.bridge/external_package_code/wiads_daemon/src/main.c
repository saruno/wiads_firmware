...
char * arg_justboot = "0";
char * api_url = "http://coreapi.wiads.net/ap/ssid";
/*=====End Variables======================================================*/
...
int main(int argc, const char * argv[]) {
	...
	system(CMD_REBOOT_MORE1DAY);
	//Xac dinh api_url. status = -1 : Chua ket noi, status = 0 : Dang ket noi, status = 1 : Ket noi success
	printf("Begin get get_api_url %s\n",mac_address);
	char * api1_url = get_api_url(1);
	char * api2_url = get_api_url(2);
	char * api1_status = get_api_status(1);
	char * api2_status = get_api_status(2);
	int indexApi = 1;
    ...
    struct http_response *hresp = http_post(api_url, NULL, params);
    if (hresp == NULL) {
    	if (arg_justboot == 0) {
    		return 0;
    	} else {
    ...
    if (strlen(content) > 0 || str_contains(params_dhcpmasq_log, "conn_station")) {
...
char* build_params_wiads_newdevice(char* dhcpmasq_log) {
	char * conn_station = get_conn_station();
	char * conn_ccu_str = get_conn_ccu();
	char * url = (char*)malloc(4096);
	strcpy(url, "mac");
	strcat(url, "=");
	strcat(url, mac_address);
	strcat(url, "&");
	if (dhcpmasq_log != NULL && strlen(dhcpmasq_log) > 0) {
		http_params_append(url, "dhcpmasq_log", dhcpmasq_log);
	}
	if (conn_station != NULL && strlen(conn_station) > 0) {
		http_params_append(url, "conn_station", conn_station);
	}
	if (conn_ccu_str != NULL && strlen(conn_ccu_str) > 0) {
		http_params_append(url, "conn_ccu", conn_ccu_str);
	}
	printf("params %s\n", url);
	return url;
}
...
void update_firmware(struct option_config* opt_config) {
	if (opt_config->update >= 1) {
	        ...
	        system(wget_cmd);
	        //upgrade firmware
	        char * upgrade_cmd = (char*)malloc(128);
...
void update_bw_profile(struct option_config* opt_config) {
	...
	int current_bw = atoi(bw_profile_str);
	int next_bw = opt_config->update;
	if (next_bw != current_bw) {
		char * down_limit = "30000";
		char * up_limit = "30000";
		if (next_bw == 1 || next_bw == 2 || next_bw == 3 || next_bw == 4) {
			down_limit = "10000";
			up_limit = "10000";
		} else if (next_bw == 5 || next_bw == 9) {
			down_limit = "5000";
			up_limit = "5000";
		} else if (next_bw == 6) {
			down_limit = "3000";
		...
		strcat(set_sqm_down_cmd, down_limit);
		strcpy(set_sqm_up_cmd, "uci set sqm.eth1.upload=");
		strcat(set_sqm_up_cmd, up_limit);
